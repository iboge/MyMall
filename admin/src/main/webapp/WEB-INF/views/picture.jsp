...
<div class="page-container">
    <%--<div class="text-c"> 日期范围：
        ...
        <input type="text" name="" id="1" placeholder=" 图片名称" style="width:250px" class="input-text">
        <button name="" id="2" class="btn btn-success" type="submit"><i class="Hui-iconfont">&#xe665;</i> 搜图片</button>
    </div>--%>
    <div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"> <a class="btn btn-primary radius" onclick="picture_add('添加轮播图','${ctx}/picture/addPicture')" href="javascript:;"><i class="Hui-iconfont">&#xe600;</i> 添加图片</a></span> <span class="r">共有数据：<strong>${requestScope.count}</strong> 条</span> </div>
    <div class="mt-20">
        <table class="table table-border table-bordered table-bg table-hover table-sort">
            ...
            <c:forEach items="${requestScope.picture}" var="picture">
                <tr class="text-c">
                    ...
                    <td>${picture.id}</td>
                    <td><a href="${picture.image}"><img width="210" class="picture-thumb" src="${picture.image}"></a></td>
                    <td>${picture.createtime}</td>
                    <td class="text-c">首页轮播图</td>
                    <c:if test="${picture.status==1}">
                        <td class="td-status"><span class="label label-success radius">已启用</span></td>
                        <td class="td-manage"><a style="text-decoration:none" onClick="picture_stop(this,${picture.id})" href="javascript:;" title="停用"><i class="Hui-iconfont">&#xe631;</i></a></td>
                    </c:if>
                    <c:if test="${picture.status==0}">
                        <td class="td-status"><span class="label label-danger radius">已停用</span></td>
                        <td class="td-manage"><a style="text-decoration:none" onClick="picture_start(this,${picture.id})" href="javascript:;" title="启用"><i class="Hui-iconfont">&#xe66b;</i></a></td>
                    </c:if>
                </tr>
            </c:forEach>
            </tbody>
