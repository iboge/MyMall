...
<div class="page-container">
    <%--<div class="text-c"> 日期范围：
        ...
        &lt;%&ndash;<input type="text" class="input-text" style="width:250px" placeholder="输入会员名称、电话、邮箱" id="" name="">&ndash;%&gt;
        <button type="submit" class="btn btn-success radius" id="1" name=""><i class="Hui-iconfont">&#xe665;</i> 搜索</button>
    </div>--%>
    <div class="cl pd-5 bg-1 bk-gray mt-20"><span class="r">共有数据：<strong>${requestScope.count}</strong> 条</span></div>
    <div class="mt-20">
        <table class="table table-border table-bordered table-hover table-bg table-sort">
            ...
            <tbody>
            <c:forEach items="${requestScope.members}" var="members">
                <tr class="text-c">
                    <td><input type="checkbox" value="1" name=""></td>
                    <td>${members.id}</td>
                    <td>${members.membername}</td>
                    <td>${members.email}</td>
                    <td>${members.createtime}</td>
                    <c:if test="${members.valid==1}">
                        <td class="td-status"><span class="label label-success radius">已启用</span></td>
                        <td class="td-manage"><a style="text-decoration:none" onClick="member_stop(this,${members.id})" href="javascript:;" title="冻结"><i class="Hui-iconfont">&#xe631;</i></a></td>
                    </c:if>
                    <c:if test="${members.valid==0}">
                        <td class="td-status"><span class="label label-danger radius">已冻结</span></td>
                        <td class="td-manage"><a style="text-decoration:none" href="javascript:;" onClick="member_start(this,${members.id})" title="启用"><i class="Hui-iconfont">&#xe66b;</i></a></td>
                    </c:if>
                </tr>
            </c:forEach>
            </tbody>
